...
describe("HTTP mock rule completion", function () {
    ...
    it("should trigger each rule once, in order, by default", async () => {
        await server.forGet("/endpoint").thenReply(200, "first response");
    ...
    it("should continue triggering the last matching rule indefinitely", async () => {
        await server.forGet("/endpoint").thenReply(200, "first response");
        await server.forGet("/endpoint").thenReply(200, "second response");
        await server.forGet("/other-endpoint").thenReply(200, "unrelated response");
    ...
    it("should run explicitly always() rules indefinitely", async () => {
        await server.forGet("/endpoint").always().thenReply(200, "endless response");
    ...
    it("should run explicitly once() rules only once", async () => {
        await server.forGet("/endpoint").once().thenReply(200, "first");
        await server.forGet("/endpoint").once().thenReply(200, "second");
    ...
    it("should run times(n) requests the given number of times", async () => {
        server.forGet("/endpoint").times(1).thenReply(200, "first");
        server.forGet("/endpoint").times(2).thenReply(200, "second/third");
    ...
    it("should show twice() endpoints as pending after the first request", async () => {
        const endpointMock = await server.forGet("/mocked-endpoint").twice().thenReply(200, "mocked data");
    ...
    it("should be used to populate pendingEndpoints", async () => {
        const firstRule = await server.forGet("/endpoint").thenReply(200, "first response");
        const secondRule = await server.forGet("/endpoint").thenReply(200, "second response");
